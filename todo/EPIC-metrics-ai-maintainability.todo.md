...
- Временный снимок из `var/metrics/` не является частью проекта: базовая ветка не хранит состояние метрик, а PR не содержит их обычный Git diff.
...
- Сформировать полный детерминированный `coding-standard-metrics.lock.json`, который всегда хранится в Git и проверяется на актуальность — TASK-metrics-tracked-snapshot.
...
- Каждая ревизия проекта хранит полный актуальный `coding-standard-metrics.lock.json`; PR содержит обычный Git diff метрик вместе с diff кода.
- Детерминированное сравнение версий lock-файла на merge-base и в текущих правках фиксирует улучшения, ухудшения, добавленные и удалённые объекты.
...
- `composer metrics` обновляет отслеживаемый lock-файл и может построить временный автономный HTML-дашборд для просмотра человеком.
...
Собрать в `prikotov/coding-standard` инструментальную цепочку для код-ревью подключаемого проекта: она создаёт полный детерминированный `coding-standard-metrics.lock.json` на уровнях «метод → класс → модуль → проект» и требует хранить его в Git как часть состояния проекта. PR обновляет lock-файл вместе с кодом, а сравнение его версий на merge-base и HEAD различает улучшения, ухудшения, добавленные и удалённые объекты. Raw diff остаётся в PR, производный отчёт входит в его артефакты и читается ИИ-агентом перед решением о прохождении ревью. HTML-дашборд остаётся дополнительным временным представлением для человека. Критерий: потребительский сценарий воспроизводится одной документированной цепочкой на TasK, одинаковые входы дают одинаковый результат, `composer check` пакета остаётся зелёным.
...
- [ ] Полный канонический `coding-standard-metrics.lock.json` хранится в Git и автоматически проверяется на актуальность (TASK-metrics-tracked-snapshot).
...
flowchart LR
    ...
    D[Агрегатор bin/metrics-aggregate.php] -->|полный снимок| L[coding-standard-metrics.lock.json в Git]
...
- Канонический lock-файл содержит всю модель в одном JSON без времени генерации, абсолютных путей и самоссылки на commit; промежуточный `var/metrics/` остаётся в `.gitignore`.
...
- [ ] [TASK-metrics-tracked-snapshot](TASK-metrics-tracked-snapshot.todo.md) — полный актуальный lock-файл метрик в Git
...
- [ ] Все Must-задачи выполнены: временный отчёт и отслеживаемый lock-файл покрывают модель метрик.
- [ ] `composer metrics` работает end-to-end одной командой в проекте-потребителе.
- [ ] `coding-standard-metrics.lock.json` содержит все метрики, хранится в Git и проверяется на соответствие текущим входам.
- [ ] Для одного PR воспроизводимо сравниваются версии lock-файла на merge-base и HEAD.
...
- [ ] Инструкция требует коммитить `coding-standard-metrics.lock.json` и проверять его актуальность в CI.
- [ ] Инструкция показывает извлечение baseline из Git, сравнение с current и сохранение артефакта PR.
- [ ] Инструкция требует добавить временный `var/metrics/` в `.gitignore` проекта-потребителя.
...
- Churn в отслеживаемом lock-файле должен одинаково учитывать подготовленные изменения до коммита и ту же ревизию после коммита.
- Полный lock-файл может быть крупным; формат должен сохранять все метрики, но оставаться стабильным и пригодным для Git diff.
...
- Для ИИ-агентов источник истины — отслеживаемый `coding-standard-metrics.lock.json`; HTML-дашборд — временное представление для людей.
...
- Git уже хранит историю lock-файла: его версии на merge-base и HEAD являются входами сравнения, а артефакт PR содержит их детерминированную дельту.
...
| 2026-08-10 | codex (Codex) | Канонический полный снимок метрик закреплён как отслеживаемый lock-файл проекта; baseline для PR берётся из его версии на merge-base. |

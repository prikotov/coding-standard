...
- Итоговая JSON-структура пока находится во временном `var/metrics/`: базовая ветка не хранит состояние метрик, а PR не содержит их обычный Git diff.
...
- Публиковать каноническую зеркальную JSON-структуру в корневой `metrics/`, хранить её в Git и проверять актуальность — TASK-metrics-tracked-snapshot.
...
- Каждая ревизия проекта хранит полную актуальную JSON-структуру `metrics/`; PR содержит обычный Git diff метрик вместе с diff кода.
...
- [ ] Канонические JSON `metrics/` хранятся в Git и автоматически проверяются на актуальность (TASK-metrics-tracked-snapshot).
...
flowchart LR
    ...
    D[Агрегатор bin/metrics-aggregate.php] -->|публикация зеркала| L[metrics/**/*.json в Git]
...
- Каноническая `metrics/` совпадает по внутреннему зеркалу с отчётами из конвенции: корневой, каталоговые и файловые JSON хранятся в Git без времени генерации, абсолютных путей и самоссылки на commit.
- HTML и промежуточные результаты анализаторов остаются во временном `var/metrics/` и не отслеживаются.
...
- [ ] Канонические JSON `metrics/` содержат все метрики, хранятся в Git и проверяются на соответствие текущим входам.
...
- [ ] Инструкция требует коммитить канонические JSON `metrics/` и проверять их актуальность в CI.
- [ ] Инструкция показывает извлечение baseline из Git, сравнение с current и сохранение артефакта PR.
- [ ] Инструкция разделяет отслеживаемые JSON из `metrics/` и временные HTML/промежуточные файлы из `var/metrics/`.
...
- Для ИИ-агентов источник истины — отслеживаемая JSON-структура `metrics/`; HTML-дашборд в `var/metrics/` — временное представление для людей.
...
| 2026-08-10 | codex (Codex) | Канонический снимок уточнён как отслеживаемая зеркальная структура `metrics/`; временные файлы и HTML остаются в `var/metrics/`. |

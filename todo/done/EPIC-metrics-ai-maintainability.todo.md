...
- Публиковать каноническую зеркальную JSON-структуру в `.coding-standard/metrics/`, хранить её в Git и проверять актуальность — TASK-metrics-tracked-snapshot.
...
- Каждая ревизия проекта хранит полную актуальную JSON-структуру `.coding-standard/metrics/`; PR содержит обычный Git diff метрик вместе с diff кода.
...
flowchart LR
    ...
    D[Агрегатор bin/metrics-aggregate.php] -->|публикация зеркала| L[.coding-standard/metrics/**/*.json в Git]
...
- Каноническая `.coding-standard/metrics/` совпадает по внутреннему зеркалу с отчётами из конвенции: корневой, каталоговые и файловые JSON хранятся в Git без времени генерации, абсолютных путей и самоссылки на commit.

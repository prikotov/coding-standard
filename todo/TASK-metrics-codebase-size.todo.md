...
- [ ] Отсутствие PCOV не роняет пайплайн: шаг пропускается с предупреждением, отчёт собирается без секции покрытия. `scc` обязателен для проверки полноты структурного отчёта.

...
| `UnusedFormalParameter` | Параметр обязателен по контракту (`__invoke`, interface, `message handler`), но полезная нагрузка (payload) пустая | Неиспользуемые аргументы в приватных методах и внутренних сервисах | Подтвердить контрактную необходимость; где можно — удалить параметр/упростить сигнатуру |

...
Проверяются только документы на русском. Документ на другом языке помечается (приоритет — front matter) и пропускается:

- front matter: `lang: en`;
- имя файла: `name.en.md`.

Без признака языка документ считается русским и проверяется. Это позволяет держать в репозитории английские справочники и переводы, не получая ложных срабатываний.

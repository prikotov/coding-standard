...
- **`ratio`** = (слова в английских фразах) / (все слова). Файлы, где `ratio` превышает порог, помечаются warning. Порог по умолчанию — **2%** (настраивается через `--threshold` или ключ `threshold` конфига).
- Одиночные английские слова вне allowlist не входят в `ratio` — ловим именно английские обороты, а не разрозненные термины.
...
# Своий порог (по умолчанию 0.02 = 2%).
vendor/bin/validate-language --threshold=0.05
...
return [
    ...
    // Порог warning (по умолчанию 0.02 = 2%).
    'threshold' => 0.05,

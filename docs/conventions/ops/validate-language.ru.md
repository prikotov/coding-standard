...
- **`ratio`** = (латинские слова вне allowlist) / (все слова). Одиночные англицизмы (например allowlist) учитываются наравне с оборотами (например persisted rows). Файлы, где `ratio` превышает порог, помечаются warning. Порог по умолчанию — **2%** (настраивается через `--max-ratio` или ключ `max_ratio` конфига).

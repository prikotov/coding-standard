...
- **`ratio`** = (латинские слова вне allowlist) / (все слова). Англицизм — любое латинское слово вне allowlist: одиночное (allowlist) или в обороте (persisted rows). Файлы, где `ratio` превышает порог, помечаются warning. Порог по умолчанию — **2%** (настраивается через `--max-ratio` или ключ `max_ratio` конфига).

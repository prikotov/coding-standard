...
- **Стартовый набор** — `coding-standard-init` записывает минимальный `dictionary` только из жаргонных/проектно-специфичных терминов с неоднозначным переводом (`hook`, `god object`); обычные слова с очевидным переводом (`parallel`, `path`, `execution`) сюда не кладутся. Потребитель дополняет под проект.
...
return [
    ...
    'language' => [
        ...
        // Стандартные переводы жаргона/проектно-специфичных терминов (на ratio не влияет).
        'dictionary' => [
            'hook' => 'хук',

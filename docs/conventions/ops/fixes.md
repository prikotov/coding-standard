...
- Фикс по умолчанию работает в режиме `dry-run`. Для применения нужен `--execute`.
- Для изменений данных обязателен `CSV`-бэкап до модификаций.
...
- Фикс по умолчанию работает в режиме `dry-run`. Для применения изменений нужен `--execute`.
- Для изменений данных обязателен `CSV`-бэкап до модификаций.
...
Запуск в окружении разработки (podman, контейнер уже запущен):
...
Запуск в окружении разработки (docker compose, сервис уже запущен в рамках compose):
...
Перед удалением убедитесь, что фикс не используется и задокументирован результат применения.

## Чек-лист для проведения ревью кода

- [ ] Фикс работает в режиме `dry-run` по умолчанию.
- [ ] `CSV`-бэкап создаётся до модификаций.

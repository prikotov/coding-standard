...
- Каждое приложение находится в `apps/<app_name>/` и наследуется от общего [`AppName\Common\Kernel`](examples/Kernel.php).
...
Namespace следует паттерну `AppName\{AppGroup}\Module\{ModuleName}\{Layer}\...`, где:

- `AppName` — корневой namespace проекта (например, `TaskOrchestrator\`). Может быть пустым, если PSR-4 маппит `Common\` напрямую.
...
- **Создание нового приложения**: создайте директорию `apps/<app_name>/` с Kernel, наследуемым от `AppName\Common\Kernel`.
...
- [ ] Namespace классов следует паттерну `AppName\{AppGroup}\Module\{ModuleName}\...`.
- [ ] Конфигурация разделена на общую (`config/`) и приложения (`apps/<app_name>/config/`).

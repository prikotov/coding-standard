...
**Окружение (Environment)** — набор конфигурационных настроек для конкретной среды выполнения (разработка, тестирование, `prod`). Подробности: [Окружения Symfony](https://symfony.com/doc/current/configuration/environments.html).
...
- Сущности Doctrine, DTO, enum, value object, события, исключения, команды/запросы не должны быть зарегистрированы как сервисы.
...
- **Исключайте несервисные типы** из автоконфигурации сервисов — Doctrine должна управлять сущностями сама, а DTO, enum, value object, события, исключения, команды/запросы не должны попадать в контейнер.

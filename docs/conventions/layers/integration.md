...
- Если реализация связывает модули внутри одного процесса и не использует внешний transport/API/SDK, размещаем её в
  `Integration\Service`.
- Если та же возможность вынесена в отдельный сервис и связь идёт через HTTP/gRPC/SDK/queue client, реализация
  размещается в `Infrastructure\Service`/`Infrastructure\Component`, даже если сервис вырос из бывшего модуля проекта.

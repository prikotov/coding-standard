...
**Application слой** — слой приложения, который оркеструет выполнение бизнес-операций, координирует взаимодействие между доменной логикой, инфраструктурой и внешними интерфейсами. Это уровень, где реализуются Use Cases приложения.
...
- **Оркестрация бизнес-операций**: координирует выполнение Use Cases, объединяя несколько операций домена в единый процесс
...
Use Case — реализует конкретное действие, которое может выполнить пользователь или система. Use Case-ы делятся на два типа:
...
Подробнее: [Use Case](application/use-case.md) | «Symfony Messenger» — [документация](https://symfony.com/doc/current/messenger.html).

### Command Handler

**Command Handler** — реализует изменение состояния приложения. Хендлер:
...
**Query Handler** — возвращает данные из приложения. Хендлер:

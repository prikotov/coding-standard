...
```php
AppName\Common\Module\{ModuleName}\Integration\Listener\{Context?}\{Action}On{EventName}Listener
```
...
- [ ] Имя по схеме `{Action}On{EventName}Listener`, namespace
  `AppName\Common\Module\{ModuleName}\Integration\Listener\{Context}`.

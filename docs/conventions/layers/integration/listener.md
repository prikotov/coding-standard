...
```php
ProjectName\Common\Module\{ModuleName}\Integration\Listener\{Context?}\{Action}On{EventName}Listener
```
...
- [ ] Имя по схеме `{Action}On{EventName}Listener`, namespace
  `ProjectName\Common\Module\{ModuleName}\Integration\Listener\{Context}`.

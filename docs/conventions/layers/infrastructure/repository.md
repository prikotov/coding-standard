...
**Репозиторий** — реализация доменного контракта репозитория, скрывающая работу с ORM/БД.
...
- В Application слое используем только доменный интерфейс; реализация не просачивается наружу.
...
- [ ] Методы репозитория работают с доменными сущностями, а не с ORM-моделями.

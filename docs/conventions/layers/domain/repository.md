...
- **Рекомендуемые методы** — если домену нужна соответствующая операция, она объявляется под следующим именем и сигнатурой (присутствие метода не обязательно: не каждой сущности нужны все операции — Interface Segregation):

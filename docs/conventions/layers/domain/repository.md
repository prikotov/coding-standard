...
- Действие этих правил — только репозитории доменных сущностей: интерфейсы, методы которых принимают и/или возвращают `*Model`. Доменный контракт, методы которого работают с другими типами (например, возвращает VO), — другой вид контракта и здесь не регламентируется; именование его методов доменное, не имитирующее выборку сущности (см. [value-object.md](../../core-patterns/value-object.md)).

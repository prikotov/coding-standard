...
- Для возврата [Value Object](../../core-patterns/value-object.md), а не сущности (`*Model`) — вычисляемого или сводного доменного значения (баланс на дату, сумма операций, статистику) — создают отдельный репозиторий, а не добавляют такой метод в репозиторий сущности.

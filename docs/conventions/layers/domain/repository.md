...
- **Рекомендуемые методы** — для типовых операций используются следующие имена и сигнатуры (набор выбирается по потребности: не каждой сущности нужны все операции — например, read-only сущности не требуют `save`/`delete`):
  ...
  - `getById(?int $id = null, ?Uuid $uuid = null): Entity` — загрузка по идентификатору или UUID, при отсутствии выбрасывает [NotFoundExceptionInterface](../../core-patterns/exception.md#интерфейсы-исключений) (**не nullable**).
...
- Репозиторий маппит исключения ORM/SDK в доменные: `NotFoundExceptionInterface` для отсутствия сущности, [InfrastructureExceptionInterface](../../core-patterns/exception.md#интерфейсы-исключений) для ошибок работы хранилища.

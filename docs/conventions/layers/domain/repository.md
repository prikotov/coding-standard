...
- Действие этих правил — только репозитории доменных сущностей: интерфейсы, методы которых принимают и/или возвращают `*Model`. Доменный контракт, методы которого работают с другими типами (например, возвращает VO), — другой вид контракта и здесь не регламентируется; имя его метода доменное, не имитирующее выборку сущности (`findById`/`getOne`/`findOne` зарезервированы за репозиторием сущностей).

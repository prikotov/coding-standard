...
- **Рекомендуемые методы** — если домену нужна соответствующая операция, она объявляется под следующим именем и сигнатурой. Набор методов выбирается по потребности: не каждой сущности нужны все операции (например, read-only сущности не требуют `save`/`delete`).

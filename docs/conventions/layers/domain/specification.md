...
- Определяется и принадлежит доменному слою (Domain).
...
- Спецификации определяются в **Domain-слое**, но могут использоваться из **Application-слоя** (Command/Query Handler) и других доменных сервисов.

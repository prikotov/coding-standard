...
- Нормализация ответа выполняется в маппере/контроллере (mapper/controller) до создания DTO, а не внутри DTO.
...
`ResponseDto` описывает только внешний контракт ответа: сериализуемые поля, типы и вложенные transport DTO.

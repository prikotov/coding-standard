...
* Событие — объявление факта, а не обязательство реакции: слушатели решают, что делать с фактом
  (логирование, метрики, фоновые задачи). Даже без слушателей событие задаёт точку расширения —
  реакцию можно добавить позже, не меняя код, объявивший факт.
...
* [ ] События диспетчеризуются **после** `flush()`, а не внутри транзакции.

...
- Мутирующий Command Handler публикует хотя бы одно событие (`*Event`); читающие Query-хендлеры события не публикуют.
...
- Мутирующий Command Handler — тот, который вызывает `save()`, `persist()`, `remove()`/`delete()`
  репозитория или менеджера персистентности либо завершается `flush()` — **обязан диспетчеризовать хотя бы
  одно событие (`*Event`)** через шину событий. Факт изменения состояния должен быть виден другим модулям.
  Проверяется warning'ом сниффа `CommandHandlerEventDispatch` (`ruleset.xml`).
- Исключения — событие не требуется:
    - внутренние служебные команды без внешних наблюдателей: аудит, счётчики, инвалидация кэша,
      пересчёт агрегатов (`Recalculate*`, `Adjust*`, `MarkUsed` и т.п.);
    - idempotent-фиксы и миграционные/поддерживающие команды;
    - осознанное исключение фиксируется комментарием
...
- [ ] Мутирующий Command Handler публикует хотя бы одно `*Event` (или исключение осознанное и задокументировано).

...
- Command Handler **диспетчеризует хотя бы одно событие (`*Event`)** через шину событий — факт изменения состояния
  ...
  Проверяется на этапе код-ревью: метрика `command_handlers_without_event`
  ([конвенция метрик](../../ops/quality-metrics.md)) помечает регрессией PR, увеличивающий количество
  хендлеров, которые меняют состояние (`save()`, `delete()`, `persist()`, `remove()`, `flush()`),
  но не содержат вызова `dispatch()`; решение «добавить событие или обосновать исключение» принимает автор PR.
...
- [ ] Command Handler публикует хотя бы одно `*Event` (либо случай попадает под исключение и обоснован в PR).

...
- Command Handler публикует хотя бы одно событие (`*Event`); событие диспетчеризуется после `flush()` (если хендлер изменяет состояние), когда данные уже в БД.
...
- Command Handler **диспетчеризует хотя бы одно событие (`*Event`)** через шину событий — объявляет факт,
  который выполнил: `SmsSentEvent`, `UsageRecalculatedEvent`, `UsersMigratedEvent`.
  Событие диспетчеризуется после успешного выполнения хендлера — после `flush()`, если хендлер изменяет состояние.
- Событие — объявление факта, а не обязательство реакции: слушатели решают, что делать (логирование, метрики,
  фоновые задачи), а при их отсутствии событие ничего не стоит. Поэтому правило без исключений.
- Хендлер без события — осознанное отклонение: обосновывается в ревью PR.
...
- [ ] Command Handler публикует хотя бы одно `*Event`; без события — отклонение, обоснованное в ревью PR.

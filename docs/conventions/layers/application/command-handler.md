...
- Command Handler публикует хотя бы одно событие (`*Event`); событие диспетчеризуется после `flush()`, когда данные уже в БД.
...
- Command Handler **диспетчеризует хотя бы одно событие (`*Event`)** через шину событий — факт изменения состояния
  должен быть виден другим модулям.
  Проверяется warning'ом сниффа `CommandHandlerEventDispatch` (`ruleset.xml`): снифф предупреждает,
  если хендлер вызывает `save()`, `delete()`, `persist()`, `remove()` или `flush()` репозитория/менеджера
  персистентности, но не содержит вызова `dispatch()`.
  Снифф не различает исключения — осознанное исключение разработчик подавляет сам (см. ниже).
- Событие не требуется, если хендлер:
    - не сохраняет состояние, а только запускает побочный эффект (отправка уведомления, постановка в очередь, вызов внешнего API);
    - выполняет внутреннюю служебную работу без внешних наблюдателей: аудит, счётчики, инвалидация кэша,
      пересчёт агрегатов (`Recalculate*`, `Adjust*`, `MarkUsed` и т.п.);
    - идемпотентно исправляет или мигрирует данные.
  Исключение фиксируется комментарием
  `// phpcs:ignore PrikotovCodingStandard.Application.CommandHandlerEventDispatch.MissingEventDispatch`
  с краткой причиной рядом.
...
- [ ] Command Handler публикует хотя бы одно `*Event` (либо исключение подавлено `phpcs:ignore` с причиной).

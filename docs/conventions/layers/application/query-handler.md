...
> 💡 В продакшн-коде рекомендуется использовать `QueryBus` для доставки запросов, особенно при использовании «Symfony Messenger» и очередей. Прямой вызов Query Handler допустим для unit-тестов или простых MVP-прототипов.

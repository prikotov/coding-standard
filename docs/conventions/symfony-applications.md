...
- **Создание нового приложения**: создайте директорию `apps/<app_name>/` с необходимой структурой и Kernel, наследуемым от [`Common\Kernel`](examples/Kernel.php).
...
- [ ] Kernel приложения наследуется от [`Common\Kernel`](examples/Kernel.php).

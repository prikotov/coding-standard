...
- При неявной ORM-транзакции `flush()` записывает изменения и фиксирует транзакцию.
  При явно открытой внешней транзакции события отправляются только после её успешного `commit()`, не сразу после `flush()`.
- Выбор CAS и блокировок, актуальность ORM и запрет внешнего ввода-вывода в транзакции описаны
...
**События отправляются после окончательной фиксации транзакции**: после `flush()` для неявной ORM-транзакции
или после внешнего `commit()` для явной. Примеры и схема ниже предполагают отсутствие внешней транзакции.
...
- [ ] События отправляются после окончательного `commit()` (либо `flush()` без внешней транзакции).

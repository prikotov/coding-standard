...
Основной принцип — DTO размещается **рядом с владельцем** (классом или группой, которая его использует) — в той же папке, что владелец, либо в её поддиректории `Dto/`. Владелец — это use case, сервис, компонент, событие, доменный объект, интеграция. Исключение — **общие переиспользуемые DTO**.

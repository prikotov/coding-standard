...
**Mapper** — класс для технического преобразования структуры данных (shape mapping) на границах слоёв/модулей и интеграций. Бизнес-логику не реализует.
...
Mapper размещается в слое, где он используется. Примеры:
...
- Mapper используется только на границах слоёв/модулей и интеграций.
- Mapper используется для адаптации контрактов между слоями/модулями и внешними форматами.
- Mapper не должен использоваться для замены бизнес-логики.
- Методы Mapper вызываются через экземпляр, полученный из DI.
- Mapper должен быть `final` и `readonly`.
...
- [ ] Нет изменений состояния БД и I/O-операций внутри Mapper.

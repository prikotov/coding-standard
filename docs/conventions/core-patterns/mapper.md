...
```
{AppName}\Common\Module\{ModuleName}\Application\Mapper\{Name}Mapper
```

**Infrastructure слой** — маппинг Criteria → QueryBuilder, DTO → массив:

```
{AppName}\Common\Module\{ModuleName}\Infrastructure\Repository\{Entity}\Criteria\Mapper\{Name}Mapper
{AppName}\Common\Module\{ModuleName}\Infrastructure\Component\{Component}\Mapper\{Name}Mapper
{AppName}\Common\Module\{ModuleName}\Infrastructure\Mapper\{Name}Mapper
```

**Domain слой** — маппинг внутри домена:

```
{AppName}\Common\Module\{ModuleName}\Domain\Mapper\{Name}Mapper
```

**Integration слой** — маппинг для внешних API:

```
{AppName}\Common\Module\{ModuleName}\Integration\Component\{Component}\Mapper\{Name}Mapper
```

**Presentation слой** — маппинг для контроллеров:

```
{AppName}\Common\Module\{ModuleName}\Presentation\Mapper\{Name}Mapper
```

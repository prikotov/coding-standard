...
```
AppName\Common\Module\{ModuleName}\Domain\Factory\{Name}Factory
```

**Application слой** — фабрики для DTO и объектов use case:

```
AppName\Common\Module\{ModuleName}\Application\Factory\{Name}Factory
AppName\Common\Module\{ModuleName}\Application\UseCase\Command\{UseCase}\{Name}Factory
AppName\Common\Module\{ModuleName}\Application\UseCase\Query\{UseCase}\{Name}Factory
AppName\Common\Module\{ModuleName}\Application\Service\{ServiceName}\{Name}Factory
```

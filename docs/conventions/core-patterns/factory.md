...
- DTO — в Application/Presentation. В Domain не создаём общие DTO; узкие DTO конкретного доменного контекста допустимы
  только по правилам [DTO](dto.md); любые классы в `Domain\Dto\*` запрещены.

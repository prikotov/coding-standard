...
- Не плодите хелпер-классы без необходимости: если утилита используется редко или логически относится к существующему хелперу, расширяйте текущий класс в рамках его узкого контекста.
...
**Запрещено**:
- Хранить состояние (stateful): статические свойства, кеши, счётчики, синглтоны.
...
- Для новых хелпер-классов `StaticAccess` обрабатывается по правилам [обоснованных `suppressions` PHPMD](../ops/phpmd-suppressions-guidelines.md): `FQCN` добавляется в `phpmd.xml` (`StaticAccess exceptions`) только при подтверждённой системной необходимости.

...
- Запрещено использовать компонент напрямую из других слоёв (`Domain`, `Application`, `Integration`, `Presentation`).
- Другие слои работают с внешним API/SDK через доменный интерфейс (`Domain\Service\*Interface`).
- Infrastructure-реализация доменного интерфейса вызывает компонент и маппит его ответ в контрактные типы (`VO`/`DTO`/`Enum`/скаляры).
...
**Логика обработки** ответа — в Infrastructure-сервисе, который реализует доменный контракт и маппит ответ компонента в его типы (`VO`/`DTO`/`Enum`/скаляры).

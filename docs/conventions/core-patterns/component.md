...
- Запрещено использовать компонент напрямую из других слоёв (`Domain`, `Application`, `Integration`, `Presentation`): внешний API/SDK вызывается через Infrastructure-реализацию доменного интерфейса, которая возвращает типы его контракта (`VO`/`DTO`/`Enum`/скаляры).

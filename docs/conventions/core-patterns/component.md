...
- Вход/выход строго типизируем: **скаляры** и/или **DTO**. Все DTO/Mapper/фабрики — часть namespace компонента.
...
- Компонент — техническая деталь Infrastructure; запрещено использовать его напрямую за пределами Infrastructure слоя.
- Допускается реализация стандартных интерфейсов (PSR/Symfony Contracts) без их переопределения.
- ❗ Исключение для декораторов стандартных контрактов (например, `HttpClientInterface`):
  декоратор именуется по имени контракта, а не по шаблону `*Component`/`*ComponentInterface`
  (пример: `SourceIngestionHttpClient` — декоратор HTTP-клиента модуля Source).
  Дублирующий собственный интерфейс метод-в-метод не создаётся: взаимозаменяемость
  с любой другой реализацией контракта — часть назначения декоратора.
- Допускаются несколько реализаций одного интерфейса (`Prod`/`Sandbox`/`Dummy`).
- По умолчанию в локальной разработке интерфейс может быть замаплен на `Dummy`/`Sandbox` реализацию, в окружении `prod` — на боевую реализацию (см. таблицу в разделе «Правила применения постфикса для именования реализации интерфейса»).
...
**Логика обработки** ответа — в Infrastructure-сервисе.

❗ Запрещается хардкодить ключи и секреты в коде компонентов. Конфигурация всегда передаётся через контейнер и параметры из файлов `.env`.
Переменные окружения определяются в локальных файлах `.env.local`, которые добавлены в `.gitignore`.
...
## Правила применения постфикса для именования реализации интерфейса
...
| Тип реализации | Постфикс        | Назначение                                                                | Где использовать             | Особенности/правила                                   | Пример имени                        |
|----------------|-----------------|---------------------------------------------------------------------------|------------------------------|-------------------------------------------------------|-------------------------------------|
| `Prod`         | *без постфикса* | Боевая реализация интеграции с внешним сервисом                           | `prod`                       | Имя отражает провайдера/транспорт                     | `TBusinessPaymentsComponent`        |

...
- Запрещено использовать компонент напрямую из других слоёв (`Domain`, `Application`, `Integration`, `Presentation`): внешний API/SDK вызывается через Infrastructure-реализацию доменного интерфейса.
...
**Логика обработки** ответа — в Infrastructure-сервисе, который реализует доменный контракт.

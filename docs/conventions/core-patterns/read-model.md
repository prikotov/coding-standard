...
денормализованные срезы. Это read-only проекция данных — она **только читается** (нет `save()`/`delete()`),
не порождается бизнес-операцией напрямую и формируется из хранилища или агрегирующего запроса, а не
persist'ится как обычная сущность. Может представлять полноценную бизнес-концепцию (баланс, скоринг, отчёт) —
просто вычисляется, а не живёт самостоятельным жизненным циклом.
...
- **Источник — проекция, а не жизненный цикл:** модель вычисляется агрегатом на стороне БД, агрегирующим запросом или (редко) PHP-агрегацией; у неё нет независимого жизненного цикла, как у persist'имой сущности — она пересчитывается, а не изменяется.
- Применяется для концепций предметной области, которые удобнее выражать как вычисляемый срез, а не как persist'имую сущность 1:1: агрегаты по измерениям, саммари за период, балансы, счётчики, денормализованные срезы для списков/отчётов.

...
> Если агрегат достаточно считать в PHP из существующих сущностей — это код Application/Domain поверх их репозиториев, без отдельной Read-модели; Read-модель имеет смысл, когда агрегат формируется на стороне хранилища или агрегирующим запросом (варианты A и B).

...
не порождается бизнес-операцией напрямую и формируется из хранилища или агрегирующего запроса.
Может представлять полноценную бизнес-концепцию (баланс, скоринг, отчёт) — просто вычисляется, а не живёт
самостоятельным жизненным циклом.

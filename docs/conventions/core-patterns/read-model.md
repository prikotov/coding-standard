...
- **Только чтение:** `final readonly`, без мутаторов (в отличие от изменяемой entity); в доменном контракте **нет** `save()`/`delete()`/`persist`. Write-side к Read-модели не применяется. Обновление материализованной проекции (если есть) — отдельная инфраструктура (проектор/event-listener), не репозиторий.

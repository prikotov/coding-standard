...
- Размещается как [сущность](../layers/domain/entity.md) (постфикс `Model`, `Domain\Entity\*`) и читается через `*ReadRepositoryInterface` + [CriteriaMapper](../layers/infrastructure/criteria-mapper.md) по правилам [репозитория](../layers/domain/repository.md). Объявляется `final readonly` и иммутабельна (без мутаторов) — в отличие от изменяемой entity.

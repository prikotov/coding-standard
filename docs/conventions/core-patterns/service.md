...
- `DTO` в Domain допустим как технический carrier в узком месте, только рядом с конкретным сервисом
  (`Domain\Service\{GroupName?}\{Name}Dto`), но не как замена доменных типов. Любые классы в `Domain\Dto\*` запрещены.
- Может зависеть от:

...
- Класс `final readonly`, свойства приватные и строго типизированные. Константы (`const`) допускаются — `public` для доменных ограничений (лимиты, точность), `private` для внутренних деталей реализации.

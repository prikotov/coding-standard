...
- Если требуется несколько форматов создания или строгий контроль инвариантов, используйте статические фабричные методы с префиксом `create*()` (например, `createFromEmail()`, `createDefault()`, `createForAgent()`) и объявляйте конструктор `private`.
...
- Инстанс-методы могут быть любыми чистыми операциями над собственными значениями: геттеры (`get*`, `is*`, `has*`), вычисления (`add()`, `merge()`, `normalize()`), операции модификации (`with*()`). Главное — без побочных эффектов и без `void`.

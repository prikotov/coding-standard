...
```
{AppName}\Common\Module\{ModuleName}\Domain\ValueObject\{Name}Vo
```

- Встраиваемые VO для ORM (Embeddable), применяемые внутри моделей сущностей:

```
{AppName}\Common\Module\{ModuleName}\Domain\Entity\ValueObject\{Name}Vo
```
...
```
{AppName}\Common\Module\{ModuleName}\Application\ValueObject\{Name}Vo
{AppName}\Common\Module\{ModuleName}\Application\Dto\{Name}DtoVo
```

- VO [слоя Presentation](../layers/presentation.md) (если необходимы объекты представления):

```
{AppName}\\Web\\Module\\{ModuleName}\\...\\ValueObject\\{Name}Vo
Api\\v1\\Module\\{ModuleName}\\...\\ValueObject\\{Name}Vo
{AppName}\\Console\\Module\\{ModuleName}\\...\\ValueObject\\{Name}Vo
```
...
```
{AppName}\Common\Module\{ModuleName}\Domain\ValueObject\{GroupName}\{Name}Vo
{AppName}\Common\Module\{ModuleName}\Domain\Entity\ValueObject\{GroupName}\{Name}Vo
```

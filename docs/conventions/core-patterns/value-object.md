...
- Доменный контракт, возвращающий VO (не сущность), использует доменное имя метода, не имитирующее выборку сущности: `findById`/`getOne`/`findOne` зарезервированы за репозиторием сущностей (см. [repository.md](../layers/domain/repository.md)). Имя метода выражает доменную операцию (например, `getBalanceOn()`, `loadTotals()`).

...
- Публичные статические методы — только фабрики `create*()` (например, `createFromEmail()`, `createDefault()`, `createEmpty()`). Если конструктор не требует валидации — используйте `new` напрямую, фабрика без семантической нагрузки не нужна.

...
- Если требуется несколько форматов создания или строгий контроль инвариантов, используйте статические фабричные методы с префиксом `create*()` (например, `createFromEmail()`, `createDefault()`, `createForAgent()`). Если фабрика контролирует инварианты — конструктор объявляйте `private`. Если фабрика — удобство (например, `createEmpty()`) — конструктор может оставаться публичным.

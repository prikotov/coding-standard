...
- Публичные статические методы — только фабрики `create*()` (например, `create()`, `createFromEmail()`, `createDefault()`). Приватные статические методы допустимы как внутренние хелперы (например, `normalizeScale()`, `validateEmail()`).

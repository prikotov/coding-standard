...
```
{AppName}\Web\Module\{ModuleName}\Wrapper\{Name}Wrapper
```

**Infrastructure слой** — для работы с инфраструктурными данными:

```
{AppName}\Common\Module\{ModuleName}\Infrastructure\Component\{Component}\{Name}Wrapper
```

**Application слой** — для представления данных в use case:

```
{AppName}\Common\Module\{ModuleName}\Application\Wrapper\{Name}Wrapper
```
...
namespace {AppName}\Web\Module\Project\Wrapper;
...
namespace {AppName}\Web\Module\Project\Form;
...
use {AppName}\Web\Module\Project\Wrapper\ProjectDisplayWrapper;

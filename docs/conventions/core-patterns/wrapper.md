...
- Без состояния (stateless), кэширование запрещено.
- Не может быть входным или возвращаемым параметром между слоями — используется только для работы на конкретном слое.
- Внутри враппера не используется внедрение зависимостей (dependency injection). Сервисы, необходимые в геттерах, передаются в конструкторе при создании объекта.
...
- Для простого интерфейса доступа к данным (сокрытие лишней вложенности, соблюдение [закона Деметры](https://en.wikipedia.org/wiki/Law_of_Demeter)).
...
- Враппер используется только внутри слоя, где создан. Не передаётся между слоями — для этого используются DTO и Mapper.
...
- [ ] Внутри враппера не используется внедрение зависимостей (сервисы через конструктор).

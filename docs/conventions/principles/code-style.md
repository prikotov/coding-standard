...
Документ описывает стандарты оформления кода в проекте. Следование этим правилам обеспечивает единообразие кода, улучшает читаемость и упрощает поддержку.

## Общие правила

- Проект следует стандарту PSR-12 как базовому.
- Используется `declare(strict_types=1)` во всех PHP-файлах.
- PHP 8.4+: типизированные свойства, `readonly`, `enum`, именованные аргументы (named arguments).
...
Проект следует стандарту [PSR-12: расширенное руководство по стилю](https://www.php-fig.org/psr/psr-12/) — это базовое руководство по оформлению PHP-кода.
...
- Используйте автоформатирование среды разработки (IDE) для соблюдения стандарта

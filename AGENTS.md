...
2. **`PHPCS`-сниффы** — автоматические проверки через PHP CodeSniffer 4.x (`src/`).
3. **Метрики качества** — CLI для анализа подключаемого проекта и автономный HTML-дашборд.
...
## Метрики проекта-потребителя

- `vendor/bin/coding-standard-metrics` предназначен для проекта, подключившего
  этот пакет, и запускается из корня такого проекта.
- Не добавляйте `composer metrics` для анализа самого репозитория
  `coding-standard`: scripts зависимостей не наследуются потребителем.
- Изменения пайплайна проверяйте автоматическим потребительским сценарием и на
  проекте TasK без изменения его отслеживаемых файлов.
- Инструкция подключения и запуска находится в разделе

...
- Главный артефакт для ИИ-агента — машиночитаемая структура `var/metrics/`:
  корневой, каталоговые и файловые JSON-отчёты. HTML-дашборд является
  дополнительным представлением снимка для человека.
- Основной сценарий код-ревью сравнивает снимок merge-base целевой ветки со
  снимком текущих правок и оценивает изменение метрик, а не одно абсолютное
  значение.

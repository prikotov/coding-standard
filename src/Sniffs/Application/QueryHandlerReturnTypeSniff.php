...
final class QueryHandlerReturnTypeSniff implements Sniff
{
    private const ERROR_FORBIDDEN_RETURN_TYPE = 'ForbiddenReturnType';

    private const DOC_REF = ' See: docs/conventions/layers/application/query-handler.md';
    ...
    public function process(File $phpcsFile, $stackPtr): void
    {
        ...
        if ($className === '' || str_ends_with($className, 'QueryHandler') === false) {
            return;
        }
    ...
    private function isHandlerPath(string $filename): bool
    {
        ...
        return str_contains($normalizedPath, '/Application/UseCase/Query/');
    }
        ...
        foreach ($typeNames as $typeName) {
            foreach (self::FORBIDDEN_RETURN_TYPE_SUFFIXES as $suffix) {
                if (
                    ...
                    $phpcsFile->addError(
                        sprintf(
                            'QueryHandler __invoke must not return %s.'
                            . ' Allowed: DTO, Enum, or scalar.' . self::DOC_REF,
                            $typeName,
                        ),
                        $pointer,

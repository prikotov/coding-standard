# Стандарт кодирования и качества для PHP-проектов
...
Главная цель инструментального контура — дать ИИ-агенту **детерминированную обратную связь о качестве его решений**. Агент должен иметь возможность запустить одинаковые проверки до и после изменения, увидеть нарушенные конвенции и изменение метрик, а затем скорректировать решение без ожидания ручного кодревью. Оценка опирается не на субъективное впечатление или непрозрачный ответ другой модели, а на версионированные правила, формулы и данные проекта.

Целевая система состоит из трёх связанных уровней:

1. **Конвенции** фиксируют архитектурные и кодовые решения понятным языком: как разделять модули и слои, где размещать сущности и какие зависимости допустимы.
...
3. **Метрики качества** показывают состояние и развитие кодовой базы, включая проблемы, которые нельзя выразить одним запретом.
...
- **модуль** — распределение размера и сложности, внутренняя и внешняя связанность, циклы и размер используемого снаружи интерфейса;
- **проект** — размер кодовой базы, состав компонентов, тесты, покрытие и изменение показателей во времени.

...
| Правило | Что проверяет |
|---|---|
| `DtoReuseRule` | Общие DTO в `Module\{M}\Application\Dto\`, на которые ссылаются только файлы одного use case'а (и ничем общим) — помечаются, им место рядом с владельцем. Не зависит от имени DTO. |

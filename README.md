...
Расчёт метрик должен быть детерминированным и воспроизводимым: одинаковые исходный код, конфигурация и версии инструментов дают одинаковый результат. Формулы и формат машиночитаемого отчёта фиксируются в коде пакета, а их изменения выпускаются с новой версией. Сырые значения отделяются от списка найденных проблем (`findings`), а пороги для блокировки `CI` вводятся явно и только после проверки на реальных проектах. Единый непрозрачный «балл качества» не используется.

...
тестов и размер кодовой базы. Они позволяют сопоставить структурные метрики с
общим числом файлов и строк. Источники проекта создаются командами:
...
готового бинарного файла.

Покрытие остаётся необязательным, потому что расширение PCOV доступно не в
каждом окружении:
...
Отсутствие `scc` завершает сбор ошибкой. Отсутствие PCOV не завершает сбор с
ошибкой: файл Clover не создаётся.
...
Без файла `scc` агрегатор завершается ошибкой. Если отсутствует Clover,
секция покрытия не попадает в отчёт. Подробная схема описана в

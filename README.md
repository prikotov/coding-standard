...
3. **Метрики качества** измеряют размер, сложность, связность и динамику кодовой базы. Они дополняют обязательные правила и рекомендации, помогая оценивать поддерживаемость и следование принципам вроде `SOLID` и `KISS` там, где нет одной однозначной проверки.

...
Главный артефакт — машиночитаемый `report.json`: ИИ-агент использует снимки
базовой ветки и текущих правок для оценки изменения качества при код-ревью.
Автономный HTML-дашборд является дополнительным представлением тех же данных
для человека.

...
Главный артефакт — машиночитаемая структура `var/metrics/`: корневой,
каталоговые и файловые JSON-отчёты по зеркалу production-путей. ИИ-агент
использует версии этой структуры из базовой ветки и текущих правок для оценки
изменения качества при код-ревью. Автономный HTML-дашборд является
дополнительным представлением тех же данных для человека.

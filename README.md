...
Главная цель инструментального контура — дать ИИ-агенту **детерминированную обратную связь о качестве его решений**. Агент запускает проверки до и после изменения, видит нарушения конвенций и изменение метрик и исправляет код до ручного ревью. Результат воспроизводим: его определяют код проекта, зафиксированные формулы и конфигурация инструментов, а не оценка другой модели.

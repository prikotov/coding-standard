...
| Правило | Что проверяет |
|---|---|
| `DtoReuseRule` | Находит DTO в общей папке модуля (`Module\{ModuleName}\Application\Dto`), которые по факту использует только один use case — и предлагает переложить их рядом с владельцем. Проверяет реальные ссылки в коде, не имя DTO. |

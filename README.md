...
| Правило | Что проверяет |
|---|---|
| `DtoReuseRule` | Находит DTO в общей папке модуля (`Module\{ModuleName}\Application\Dto`), которые по факту использует только один use case — таким место рядом с владельцем, а не в общем пуле. Проверяет реальные ссылки в коде, не имя DTO. |

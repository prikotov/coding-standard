...
Расчёт метрик должен быть детерминированным и воспроизводимым: одинаковые исходный код, конфигурация и версии инструментов дают одинаковый результат. Инструменты показывают измеренные значения и отклонения, требующие внимания. Критерии оценки задаются явно и проверяются на реальных проектах.

Развитие инструментального контура ведётся в эпике [`EPIC-metrics-ai-maintainability`](todo/EPIC-metrics-ai-maintainability.todo.md). Результаты исследования формул, готовых анализаторов, диагностических инструментов и архитектуры сбора метрик собраны в [отчёте о выборе инструментов метрик](docs/research/metrics-tools-evaluation.md).

### Сбор источников метрик

Обязательные источники — структурный отчёт, полный граф Deptrac, статистика
тестов, размер кодовой базы и покрытие. Они позволяют сопоставить структурные
метрики с общим числом файлов и строк и оценить, какая часть production-кода
проверяется тестами. Источники проекта создаются командами:
...
Утилита `scc` является обязательной внешней зависимостью контура метрик. Её
устанавливают командой `go install github.com/boyter/scc/v3@latest` или из
готового бинарного файла. Для сбора покрытия требуется расширение PCOV,
устанавливаемое командой `pecl install pcov`.
Отсутствие `scc` или PCOV завершает сбор метрик ошибкой; эти инструменты не
входят в `composer check` и устанавливаются только в окружении сбора метрик.

### Агрегация метрик

`metrics-aggregate.php` не запускает анализаторы. Он читает подготовленные
источники и объединяет их в итоговый `var/metrics/report.json`:
...
Без файлов `scc` или Clover агрегатор завершается ошибкой. Подробная схема описана в
...
| `metrics-scc` | Запускает обязательный внешний `scc`, сохраняет размер кодовой базы и версию анализатора |
| `metrics-coverage` | Запускает PHPUnit с обязательным PCOV и создаёт `var/metrics/clover.xml` |

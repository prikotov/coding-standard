...
Обязательные источники — структурный отчёт, полный граф Deptrac и статистика
тестов. Структурный отчёт и статистика тестов создаются командами:
...
Размер кодовой базы и покрытие — необязательные источники, потому что внешний
`scc` и расширение PCOV доступны не в каждом окружении. Для полного отчёта их
собирают до запуска агрегатора:
...
Отсутствие `scc` или PCOV не завершает сбор с ошибкой: соответствующий файл не
создаётся.
...
Если файл `scc` или Clover отсутствует, соответствующая секция не попадает в
отчёт. Подробная схема описана в
